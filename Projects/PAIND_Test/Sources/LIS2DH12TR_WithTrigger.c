...
void accelTest(void){
	...
	WAIT1_WaitCycles(25);
	res = LIS2DH12TR_WithTrigger_ReadReg(0x0f, &data, sizeof(data));
	WAIT1_WaitCycles(25);
	res = LIS2DH12TR_WithTrigger_WriteReg(0x20, 0b01111111);
	WAIT1_WaitCycles(25);
	res = LIS2DH12TR_WithTrigger_WriteReg(0x23, 0b00100000);

	for(;;){
		WAIT1_WaitCycles(25);
		res = LIS2DH12TR_WithTrigger_ReadReg(0x29, &dataX, 1U);
...
void getAccelValue(void){
	...
	//CS1_EnterCritical();
	//WAIT1_WaitCycles(25);
	res = LIS2DH12TR_WithTrigger_ReadReg(0x29, &data_OUT_X_H, 1U);
	if(res != ERR_OK){
		return;
	}

	SetTrigger(MEASURE_ACCEL, 5U, getAccelValue, NULL);
...
void LIS2DH12TR_WithTrigger_init(void){
	...
	res = LIS2DH12TR_WithTrigger_ReadReg(0x0f, &data, sizeof(data));	// I2C test line 1
		if (res!=ERR_OK) {
			return;
		} else if(data != 0x33){
